...
# -------------------------
# Function to stop any running service in a folder
# -------------------------
function Stop-ServiceIfRunning {
    ...
    foreach ($p in $javaProcesses) {
        try {
            ...
            # If the process command line contains the service folder path, stop it
            if ($cmdLine -and $cmdLine.ToLower().Contains($Path.ToLower())) {
                Write-Host "Stopping existing process $($p.Id) for path $Path" -ForegroundColor Yellow
                Stop-Process -Id $p.Id -Force
            }
        } catch {
            Write-Host "Could not check/stop process $($p.Id): $_" -ForegroundColor Red
        }
    }
}
...
function Start-Service {
    ...
    Stop-ServiceIfRunning -Path $Path
    ...
    Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$Path'; Write-Host 'Starting $ServiceName...' -ForegroundColor Yellow; mvn clean install; mvn spring-boot:run 2>&1 | Tee-Object -FilePath '$LogFile'"
}
